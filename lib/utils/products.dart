...
class AllData {
  ...
  static List<Map<String, dynamic>> allBrands = allProduct
      .map((e) => {'brand': e['brand'].toString(), 'image': e['thumbnail']})
      .toSet()
      .toList();

  static List tmpCategories =
      allProduct.map((e) => e['category']).toSet().toList();

  static int counter = 0;

  static List<Map<String, dynamic>> allCategories = tmpCategories
      .map((e) {
        counter++;
        return {'id': counter, 'category': e};
      })
      .toSet()
      .toList();

  static List<Map> cartProducts = [];
}
